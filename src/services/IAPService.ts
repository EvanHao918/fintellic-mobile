...
import { SubscriptionType, REAL_PRODUCT_CONFIG } from '../types/subscription';
...
class IAPService {
  ...
  async loadProducts(retryCount: number = 0): Promise<void> {
    try {
      const productIds = Platform.select({
        ios: Object.values(REAL_PRODUCT_CONFIG.ios),
        android: Object.values(REAL_PRODUCT_CONFIG.android),
      }) || [];
  ...
  private getProductId(type: SubscriptionType): string {
    const config = Platform.select({
      ios: REAL_PRODUCT_CONFIG.ios,
      android: REAL_PRODUCT_CONFIG.android,
    });
    ...
    const key: keyof typeof config = type === SubscriptionType.MONTHLY ? 'monthly' : 'yearly';
